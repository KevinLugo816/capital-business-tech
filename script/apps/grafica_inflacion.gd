...
@export var grosor_linea: float = 4.0
...
func _draw() -> void:
	...
	for i in range(lineas_guia + 1):
		...
		draw_line(Vector2(MARGEN_IZQUIERDO, y_pos), Vector2(size.x - MARGEN_DERECHO, y_pos), color_red, 1.5)
		
		var valor_porcentaje: float = max_valor * porcentaje_nivel * 100.0
		var texto_porcentaje: String = str(snapped(valor_porcentaje, 1.0)) + "%"
		draw_string(fuente_default, Vector2(5.0, y_pos + 4.0), texto_porcentaje, HORIZONTAL_ALIGNMENT_LEFT, -1, 12, color_texto)

	if datos.size() < 2:
		var y_base: float = MARGEN_SUPERIOR + alto_util
		draw_line(Vector2(MARGEN_IZQUIERDO, y_base), Vector2(size.x - MARGEN_DERECHO, y_base), color_linea, grosor_linea)
		draw_circle(Vector2(MARGEN_IZQUIERDO, y_base), 6.0, color_linea)
		return
	...
	for punto in puntos:
		draw_circle(punto, 6.0, color_linea)
		draw_circle(punto, 3.0, Color.WHITE)
