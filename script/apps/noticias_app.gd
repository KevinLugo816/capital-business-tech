...
func _exit_tree() -> void:
	if EconomiaGlobal.inflacion_cambiada.is_connected(_on_inflacion_cambiada):
...
func _procesar_noticias_del_dia() -> void:
	...
	var dias_para_alquiler: int = 5 - (EconomiaGlobal.dia_actual % 5)
	if dias_para_alquiler == 5:
		noticias_secundarias.append("• [b]Recordatorio Inmobiliario:[/b] ¡Hoy vence el cobro del alquiler del local!")
	elif dias_para_alquiler == 1:
		noticias_secundarias.append("• [b]Recordatorio Inmobiliario:[/b] Mañana se cobrará la cuota de alquiler.")
